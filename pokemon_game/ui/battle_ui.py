"""
battle_ui.py — Interface utilisateur du combat Panda3D.
Barres HP, menus d'actions, messages.
"""
...
class BattleUI:
    """
    UI de combat Pokémon avec :
    - Barres HP (joueur + ennemi)
    - Noms et niveaux
    - Menu d'actions (Attaquer / Switch / Capturer / Fuir)
    - Menu des attaques
    - Zone de messages
    """
    ...
    def setup(self):
        """Initialise tous les éléments UI."""
    ...
    def _create_player_info(self):
        """Crée le panneau d'info du Pokémon joueur (bas droite)."""
    ...
    def _create_enemy_info(self):
        """Crée le panneau d'info du Pokémon ennemi (haut gauche)."""
    ...
    def _create_action_menu(self):
        ...
        DirectButton(
            text="⚔️ Attaquer",
            frameColor=(0.8, 0.3, 0.2, 1),
            pos=(-0.25, 0, 0.15),
            command=self.show_move_menu,
            parent=self.action_frame,
            **button_style
        )
        DirectButton(
            text="🔄 Switch",
    ...
    def show_move_menu(self):
        ...
        for i, move in enumerate(poke.moves):
            ...
            type_colors = {
                ...
                "water": (0.2, 0.5, 1, 1),
                "grass": (0.2, 0.8, 0.3, 1),
                "electric": (0.9, 0.8, 0.1, 1),
                "normal": (0.6, 0.6, 0.5, 1),
                "poison": (0.6, 0.2, 0.7, 1),
                "psychic": (0.9, 0.3, 0.6, 1),
                "ice": (0.4, 0.7, 0.9, 1),
                "fighting": (0.7, 0.2, 0.15, 1),
                "ground": (0.7, 0.6, 0.3, 1),
                "flying": (0.5, 0.5, 0.8, 1),
                "bug": (0.5, 0.6, 0.2, 1),
                "rock": (0.6, 0.5, 0.3, 1),
                "ghost": (0.4, 0.3, 0.5, 1),
                "dragon": (0.4, 0.2, 0.8, 1),
                "dark": (0.3, 0.2, 0.2, 1),
                "steel": (0.5, 0.5, 0.6, 1),
                "fairy": (0.8, 0.4, 0.6, 1),
            }
        ...
        DirectButton(
            ...
            pos=(0, 0, -0.2),
            command=self.show_action_menu,
            parent=self.move_frame
        )

    def show_switch_menu(self, forced=False):
        """Affiche le menu de switch."""
        self._hide_all_menus()
        ...
        DirectLabel(
            text="Choisissez un Pokémon" if not forced else "⚠️ Choisissez un Pokémon !",
            text_fg=(1, 1, 1, 1), text_scale=0.06,
            frameColor=(0, 0, 0, 0),
            pos=(0, 0, 0.3),
        ...
        for i, poke in enumerate(self.battle.team_player):
            ...
            color = (0.3, 0.3, 0.3, 1) if poke.is_fainted() else (
                (0.2, 0.5, 0.8, 1) if is_active else (0.3, 0.6, 0.3, 1)
            )
            ...
            btn = DirectButton(
                ...
                pos=(0, 0, y),
                command=self._on_switch_selected,
                extraArgs=[idx],
                parent=self.switch_frame
            )
            if poke.is_fainted() or is_active:
                btn["state"] = 0  # Désactivé
    ...
    def _on_move_selected(self, move_index: int):
        """Callback quand une attaque est sélectionnée."""
        self._hide_all_menus()
        self.scene.on_attack(move_index)

    def _on_switch_selected(self, pokemon_index: int):
        """Callback quand un Pokémon de switch est sélectionné."""
    ...
    def show_message(self, text: str):
        """Affiche un message dans la zone message."""
        if self.message_text:
            self.message_text["text"] = text

    def update_display(self):
        """Met à jour les barres HP et noms."""
